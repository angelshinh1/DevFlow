      ...
      <div className="flex flex-col rounded-[calc(var(--radius-card)-1px)] bg-surface" style={{maxHeight: "calc(100dvh - 5rem)"}}>
        <div className="flex shrink-0 items-center gap-2.5 border-b border-line px-5 py-4">
          ...
          <div className="flex-1">
            <h2 className="text-sm font-semibold text-fg">AI review</h2>
  ...
  return (
    <div className="flex flex-col items-center gap-4 py-8 text-center">
      ...
      <div className="space-y-1">
        <p className="text-sm font-medium text-fg">No review yet</p>
        <p className="mx-auto max-w-xs text-sm text-fg-muted">
          Generate a structured review of this PR — summary, likely bugs, and suggestions with
          severity ratings.
        </p>
      </div>
...
function GeneratingState() {
  return (
    <div className="flex flex-col items-center gap-4 py-10 text-center">
      ...
      <div className="space-y-1">
        <p className="text-sm font-medium text-fg">Analyzing the diff…</p>
    ...
    <div className="flex flex-col gap-6">
      <section>
        <div className="mb-2 flex items-center justify-between">
          <h3 className="text-xs font-semibold uppercase tracking-wider text-fg-subtle">Summary</h3>
          <SeverityBadge severity={review.severity} />
        </div>
        <p className="text-sm leading-relaxed text-fg-muted">{review.summary}</p>
      </section>
      ...
      <section>
        ...
        {review.bugs.length === 0 ? (
          <p className="text-sm text-fg-muted">No likely bugs found in this diff.</p>
        ) : (
          <ul className="flex flex-col gap-2.5">
            {review.bugs.map((bug, i) => (
              <li key={i} className="rounded-lg border border-line bg-surface-raised p-3.5">
                <div className="flex items-start justify-between gap-3">
                  <p className="text-sm font-medium text-fg">{bug.title}</p>
                  <SeverityBadge severity={bug.severity} className="shrink-0" />
                </div>
                <p className="mt-1 text-sm leading-relaxed text-fg-muted">{bug.description}</p>
                {bug.location && (
      ...
      <section>
        ...
        {review.suggestions.length === 0 ? (
          <p className="text-sm text-fg-muted">No suggestions — nice and clean.</p>
        ) : (
          <ul className="flex flex-col gap-2.5">
